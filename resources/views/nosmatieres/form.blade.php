...
<div class="col-md-6 mb-3 {{ $errors->has('classe_id') ? 'has-error' : ''}}">
    <label for="formateur_id" class="control-label">{{ 'Classes' }}</label>
    <select class="form-control" name="classe_id"  id="classe_id" >
        <option value="" >-- Choisir une Classe --</option>
        @foreach($classes as $classe)
                <option value="{{ $classe->id }} {{ isset($matieres->classe_id) && $matieres->classe_id == $formateur->id ? 'selected' : ''}}">{{ $classe->name}}</option>
        @endforeach
    </select>
